<template>
  <div class="app-container">
    <el-card class="layout-title" shadow="never">
      ...
      <span style="margin-top: 5px">照片类型</span>
    ...
    <div class="table-container">
      <el-table ref="userList" style="width: 100%" :data="list" v-loading="listLoading" border>
        ...
        <el-table-column label="语音包链接" align="center">
          <template slot-scope="scope">{{scope.row.sound}}</template>
        </el-table-column>
...
import { getPotList, setPotDelete } from "@/api/baseSetting";
const contentList = {
  id: "",
  name: "",
  sound: '',
  ...
  methods: {
    ...
    handleDelete: function(row) {
      this.$confirm("此操作将永久删除该角色, 是否继续?", "提示", {
        ...
        .then(() => {
          setPotDelete({ id: row.id }).then(res => {
            if (res.code === 0) {
              this.getList();
