<template>
  <div class="app-container">
    <el-card class="layout-title" shadow="never">
      ...
      <span style="margin-top: 5px">角色赋权</span>
      ...
      <el-table ref="userList" style="width: 100%" :data="list" v-loading="listLoading" border>
        ...
        <el-table-column label="权限" align="center">
          <template slot-scope="scope">
            <el-button size="mini" @click="showModelEvent(scope.row)">查看</el-button>
          </template>
        </el-table-column>
...
import { getRoleList, setRoleDelete } from "@/api/baseSetting";
const contentList = {
  id: "",
  permission: [],
  role: ""
  ...
  methods: {
    ...
    handleDelete: function(row) {
      this.$confirm("此操作将永久删除该角色, 是否继续?", "提示", {
        ...
        .then(() => {
          setRoleDelete({ id: row.id }).then(res => {
            if (res.code === 0) {
              this.getList();
